...
window.addEventListener('DOMContentLoaded', _ => {
  ...
  rerunButton.addEventListener('click', () => {
    ...
    rerunLighthouse().then(newReportUrl => location.assign(newReportUrl));
  });
});
 ...
 * @return {!Promise} resolve to new report URL after rerun is completed
 */
function rerunLighthouse(additionalFlags={}) {
  const id = window.location.href.match(/&|\?id=(\d+)/)[1] || 0;
  return fetch(`/rerun?id=${id}`, {method: 'POST', body: JSON.stringify(additionalFlags)})
